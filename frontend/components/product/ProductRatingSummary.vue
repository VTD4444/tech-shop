...
<template>
  <div class="flex flex-wrap items-center gap-4">
    <div class="text-center">
      <div class="text-3xl font-bold text-text-primary">{{ average.toFixed(1) }}</div>
      <ProductRatingStars :model-value="Math.round(average)" readonly />
      <div class="text-sm text-text-muted mt-1">{{ count }} đánh giá</div>
    </div>
    <div v-if="distribution" class="flex-1 min-w-[160px] space-y-1">
      <div v-for="star in [5, 4, 3, 2, 1]" :key="star" class="flex items-center gap-2 text-xs">
        <span class="w-8 text-text-muted">{{ star }}★</span>
        ...
        <span class="w-6 text-text-muted text-right">{{ distribution[String(star)] || 0 }}</span>
      </div>
    </div>
  </div>
</template>
