...
<template>
  <header class="sticky top-0 z-40 border-b border-subtle bg-surface-1/95 backdrop-blur-md">
    ...
    <div v-if="mobileOpen" class="lg:hidden border-t border-subtle bg-surface-1 px-4 py-4 space-y-3">
      ...
      <template v-if="authStore.isAuthenticated">
        <p class="text-xs text-text-muted uppercase tracking-wide pt-2">Tài khoản</p>
        <NuxtLink to="/forgot-password" class="block text-sm text-text-muted hover:text-accent py-1" @click="closeMobile">
          Đổi mật khẩu
        </NuxtLink>
        <NuxtLink to="/orders" class="block text-sm text-text-muted hover:text-accent py-1" @click="closeMobile">
          Đơn hàng
        </NuxtLink>
        <NuxtLink to="/wishlist" class="block text-sm text-text-muted hover:text-accent py-1" @click="closeMobile">
          Yêu thích của tôi
        </NuxtLink>
        <NuxtLink v-if="authStore.isAdmin" to="/admin" class="block text-sm text-text-muted hover:text-accent py-1" @click="closeMobile">
          Quản trị
        </NuxtLink>
